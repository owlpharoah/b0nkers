...
impl Bonker {
    ...
    pub fn set_host_name(&self) {
        sethostname("bonker").unwrap();
    }
    pub fn customize_limits(&self, max_memory: u32, max_pids: u32) {
        let id = self.id.clone();
        let cgroup = PathBuf::from("/sys/fs/cgroup").join(format!("oyster-{id}"));
        fs::create_dir_all(&cgroup).unwrap();

        fs::write(cgroup.join("memory.max"), (max_memory).to_string()).unwrap();
        fs::write(cgroup.join("pids.max"), max_pids.to_string()).unwrap();
        let pid = std::process::id();
        fs::write(cgroup.join("cgroup.procs"), pid.to_string()).unwrap();
    }

    // will also initlize root at merged
    pub fn initlize_fs(&self) {
        let id = self.id.clone();
...
fn main() {
    ...
    fs::remove_dir(PathBuf::from("/sys/fs/cgroup").join(format!("oyster-{id}"))).unwrap();
